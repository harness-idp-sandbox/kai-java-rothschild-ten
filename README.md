...
# run app
java -jar target/java-rothschild-ten-1.0-SNAPSHOT.jar
...
docker buildx build   --platform linux/amd64   -t parsontodd/java-rothschild-ten:latest   --push .
```
...
# watch rollout
kubectl rollout status deploy/java-rothschild-ten -n kaiser

# get external endpoint
kubectl get svc java-rothschild-ten-svc -n kaiser
```
...
```bash
kubectl port-forward svc/java-rothschild-ten-svc 8080:80 -n kaiser
...
kubectl rollout restart deploy/java-rothschild-ten -n kaiser
kubectl rollout status deploy/java-rothschild-ten -n kaiser
```
...
info.app.name=javarothschildten
...
├─ src/
│  ├─ main/
│  │  ├─ java/com/harness/javarothschildten/
│  │  │  ├─ App.java
...
│  │        └─ index.html
│  └─ test/
│     └─ java/com/harness/javarothschildten/AppTest.java
├─ kubernetes/
│  ├─ deployment.yml
│  └─ service.yml
├─ Dockerfile
